...
function dynCLBM_D2Q9:relaxing_moments()
  ...
  return {
    {m = PxyMoment, tau = model.w, eq = 0 --[[eq = Ux*Uy                 ]]},
    {m = NMoment, tau = model.w, eq = 0 --[[eq = Ux^2-Uy^2             ]]},
    {m = TMoment, tau = model.wb, eq = 2 / 3 --[[eq = 2/3+(U..U)            ]]},
    {m = QxyyMoment, tau = model.w3, eq = 0 --[[eq = Ux*(1/3+Uy^2)         ]]},
    {m = QyxxMoment, tau = model.w3, eq = 0 --[[eq = Uy*(1/3+Ux^2)         ]]},
    {m = AMoment, tau = model.w4, eq = 1 / 9 --[[eq = (1/3+Ux^2)*(1/3+Uy^2) ]]}
  }
end
...
function dynCLBM_D2Q9:genDFComputations()
  ...
  for i = 1, node.Q do
    ...
    if self:mask(ei, 0, 0) then
      formula =
        rho *
        (1 - (U .. U) + 4 * Ux * Uy * Pxy - (Ux ^ 2 - Uy ^ 2) / 2 * N + ((U .. U) - 2) / 2 * T + 2 * Ux * Qxyy +
          2 * Uy * Qyxx +
          A)
    elseif self:mask(ei, 1, 0) then
      formula =
        rho / 2 *
        (Ux ^ 2 + s * Ux * (1 - Uy ^ 2) - (2 * s * Uy + 4 * Ux * Uy) * Pxy + (1 + s * Ux + Ux ^ 2 - Uy ^ 2) / 2 * N +
          (1 - s * Ux - (U .. U)) / 2 * T -
          (s + 2 * Ux) * Qxyy -
          2 * Uy * Qyxx -
          A)
    elseif self:mask(ei, 0, 1) then
      formula =
        rho / 2 *
        (Uy ^ 2 + l * Uy * (1 - Ux ^ 2) - (2 * l * Ux + 4 * Ux * Uy) * Pxy + (-1 - l * Uy + Ux ^ 2 - Uy ^ 2) / 2 * N +
          (1 - l * Uy - (U .. U)) / 2 * T -
          (l + 2 * Uy) * Qyxx -
          2 * Ux * Qxyy -
          A)
    elseif self:mask(ei, 1, 1) then
      formula =
        rho / 4 *
        (s * l * Ux * Uy + s * Ux * Uy ^ 2 + l * Uy * Ux ^ 2 + (4 * Ux * Uy + s * l + 2 * s * Uy + 2 * l * Ux) * Pxy +
          (-1 * Ux ^ 2 + Uy ^ 2 - s * Ux + l * Uy) / 2 * N +
          ((U .. U) + s * Ux + l * Uy) / 2 * T +
          (s + 2 * Ux) * Qxyy +
          (l + 2 * Uy) * Qyxx +
          A)
