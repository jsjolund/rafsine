...
computeBGK(const int x,
  ...
  dftmp3D(5, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f5 + (1.0 / tau) * f5eq + 0.5 * gBetta * (T - Tref);
  dftmp3D(6, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f6 + (1.0 / tau) * f6eq - 0.5 * gBetta * (T - Tref);
  ...
  dftmp3D(10, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f10 + (1.0 / tau) * f10eq;
  dftmp3D(11, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f11 + (1.0 / tau) * f11eq;
  dftmp3D(12, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f12 + (1.0 / tau) * f12eq;
  dftmp3D(13, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f13 + (1.0 / tau) * f13eq;
  dftmp3D(14, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f14 + (1.0 / tau) * f14eq;
  dftmp3D(15, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f15 + (1.0 / tau) * f15eq;
  dftmp3D(16, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f16 + (1.0 / tau) * f16eq;
  dftmp3D(17, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f17 + (1.0 / tau) * f17eq;
  dftmp3D(18, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tau) * f18 + (1.0 / tau) * f18eq;
  ...
  Tdftmp3D(0, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T0 + (1.0 / tauT) * T0eq;
  Tdftmp3D(1, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T1 + (1.0 / tauT) * T1eq;
  Tdftmp3D(2, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T2 + (1.0 / tauT) * T2eq;
  Tdftmp3D(3, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T3 + (1.0 / tauT) * T3eq;
  Tdftmp3D(4, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T4 + (1.0 / tauT) * T4eq;
  Tdftmp3D(5, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T5 + (1.0 / tauT) * T5eq;
  Tdftmp3D(6, x, y, z, nx, ny, nz) =
      (1.0 - 1.0 / tauT) * T6 + (1.0 / tauT) * T6eq;
