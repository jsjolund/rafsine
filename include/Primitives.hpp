...
#define PRINT_ERROR(texte)                                                     \
  {                                                                            \
    std::cerr << "Error in " << __PRETTY_FUNCTION__ << " in file " << __FILE__ \
              << ", line " << __LINE__ << ": " << texte << std::endl;          \
  }
/// Macro to print an error message and terminate the program.
#define FATAL_ERROR(texte)                                               \
  {                                                                      \
    std::cerr << "FATAL ERROR in " << __PRETTY_FUNCTION__ << " in file " \
              << __FILE__ << ", line " << __LINE__ << ": " << texte      \
              << std::endl;                                              \
    exit(-1);                                                            \
  }
...
#define PRINT_WARNING(texte)                                         \
  {                                                                  \
    std::cerr << "Warning in " << __PRETTY_FUNCTION__ << " in file " \
              << __FILE__ << ", line " << __LINE__ << ": " << texte  \
              << std::endl;                                          \
  }
#else
...
#define PRINT_UNFINISHED()                                                  \
  {                                                                         \
    std::cerr << "Warning : " << __PRETTY_FUNCTION__ << " in file "         \
              << __FILE__ << ", line " << __LINE__ << ", is not finished. " \
              << std::endl;                                                 \
  }
...
// #define CHECK_OPENGL_ERROR() (checkOpenGLerrors(__PRETTY_FUNCTION__,
// __FILE__, __LINE__))

// /// Function to check OpenGL error and print them.
// /** Do not use this function directly, use the macro CHECK_OPENGL_ERROR
// instead. */ inline void checkOpenGLerrors(std::string function, std::string
// file, int line)
// {
//   GLenum error;
//   while ((error = glGetError()) != GL_NO_ERROR)
// #ifdef NO_GLU
//     std::cerr << "OpenGL error in " << function << " in file " << file << ",
//     line " << line << ": " << error << std::endl;
// #else
//     std::cerr << "OpenGL error in " << function << " in file " << file << ",
//     line " << line << ": "
...
struct vec3 {
  ...
  static inline vec3<U> normalize(const vec3<U> &v) {
    U norm = v.norm();
    if (norm == 0) PRINT_WARNING("Vector is zero")
...
struct col3T {
  ...
  // void loadAsBackground() const { glClearColor(r / 255., g / 255., b / 255.,
  // 0); }
#endif
  /// Output a color
  template <typename U>
...
inline col3T<T> operator+(const col3T<T> &c1, const col3T<T> &c2) {
  return col3T<T>(min(255, c1.r + c2.r), min(255, c1.g + c2.g),
                  min(255, c1.b + c2.b));
}
template <typename T>
...
template <typename T>
inline col3T<T> operator*(const col3T<T> &c1, const col3T<T> &c2) {
  return col3T<T>(min(int(255), c1.r *c2.r), min(int(255), c1.g *c2.g),
...
struct pol3 {
  ...
  inline void setPhi(real phi) {
    this->phi = phi;
    while (this->phi < 0) this->phi += 2 * M_PI;
    while (this->phi > 2 * M_PI) this->phi -= 2 * M_PI;
  }
  /// compute the corresponding cartesian coordinates
  inline vec3r getPos() const {
    return vec3r(r * sin(theta) * cos(phi), r * sin(theta) * sin(phi),
                 r * cos(theta));
  }
...
struct quad {
  ...
  /// Constructor
  quad(){};
  quad(vec3r point1, vec3r point2, vec3r point3, vec3r point4)
  ...
  bool intersect(ray<vec3r> ray, real &distToIntersection) const {
    ...
    real xProj = (p - p1) * x;
    if ((xProj < 0) || (xProj > (p2 - p1).norm())) return false;
    real yProj = (p - p1) * y;
    if ((yProj < 0) || (yProj > (p4 - p1).norm())) return false;
    return true;
  }
#ifdef WITH_OPENGL
