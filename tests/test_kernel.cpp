...
__device__ __forceinline__ void runKernel(const int x, const int y, const int z,
                                          const int nx, const int ny,
                                          const int nz, real *__restrict__ df,
                                          int scl) {
  real value = (1 + I3D(x, y, z, nx, ny, nz)) * scl;
...
__global__ void TestKernel(SubLattice subLattice, real *__restrict__ df,
  ...
  runKernel(p1.x, p1.y, p1.z, arrSize.x, arrSize.y, arrSize.z, df, scl);
}
...
void runTestKernel(DistributionArray<real> *df, SubLattice subLattice, int scl,
  ...
  for (int q = 0; q < df->getQ(); q++) {
    TestKernel<<<gridSize, blockSize, 0, stream>>>(
        subLattice, df->gpu_ptr(subLattice, q), scl * (q + 1));
