...
__host__ __device__ void LatticeHistogramXform::operator()(real* input,
                                                           int i,
                                                           int* res_idx,
                                                           int* res,
                                                           int nres) const {
  real r = (input[i] - m_min) / (m_max - m_min) * (m_numBins - 1) + 0.5;
...
void LatticeHistogram::calculate(thrust::device_vector<real>* src,
                                 real min,
                                 real max,
                                 int numBins,
  ...
  real* srcPtr = thrust::raw_pointer_cast(&(*src)[0]);
